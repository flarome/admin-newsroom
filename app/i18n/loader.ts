...
type TranslationType =
  ...
  | { type: "path"; value: string }; // Chemin pour import() - new URL(`../locales/${lang}.json`, import.meta.url).pathname
...
>(config: Config): LangLoader<Lang> {
  ...
  if (config.fallback && !loader[config.fallback as Lang]) {
    if (config.translations[config.fallback]) {
      ...
      throw new Error("Fallback language must be defined in translations");
    }
  }
...
function createLangLoadFunction<Lang extends string>(
  ...
  if (!translation) {
    return () =>
      Promise.reject(
        new Error(`No translation provided for language "${lang}"`),
      );
  }
  ...
  switch (translation.type) {
    ...
    case "path":
      return () => import(/* @vite-ignore */ translation.value);

    default:
      return () =>
        Promise.reject(
          new Error(`Unsupported translation type for language "${lang}"`),
        );
  }
}

export async function loadLang<Lang extends string>(
  ...
  if (!langToUse) {
    throw new Error(
      `No available language or fallback defined for requested "${requestedLang}"`,
    );
  }

  if (!loader[langToUse]) {
    console.error(`[i18n] Loader for language "${langToUse}" is missing.`);
    throw new Error(
      `No available language or fallback defined for requested "${requestedLang}"`,
    );
  }
