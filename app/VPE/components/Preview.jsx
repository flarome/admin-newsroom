...
export default function Preview({}) {
  ...
  const { fields } = useArticle();

 
  const { theme, staticData } = useArticle();
      ...
      if (res?.path) {
        if (res.path !== prevUrlRef.current || theme.id !== prevThemeId.current) {
          ...
          if (iframeRef.current) {
            ...
            URL_CURRENT.searchParams.append('_cd', crypto.getRandomValues(new Uint32Array(1))[0]);
            URL_CURRENT.searchParams.append('_uid', crypto.randomUUID());
            URL_CURRENT.searchParams.append('preview_token', crypto.randomUUID());
