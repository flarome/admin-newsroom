...
import { form as formFieldMap } from "../../../config/fieldMap";
...
import { validateSrc } from "../../../form/validate/image";
import {
  useFormContext,
  Controller,
  useController,
...
const ImageModalContent = memo(function ImageModalContent({
  ...
  const { control, setError, getValues, reset, handleSubmit: rhfHandleSubmit } = useFormContext();
  ...
  const handleSubmit = rhfHandleSubmit(
    useCallback(async () => {
      ...
      const errorsList = await validateSrc(requireImage, data, validImageTypes);
      ...
      onValid(data);
    }, [getValues, requireImage, validImageTypes, setError, onValid])
  );
  ...
  return (
    <Modal id={modalId} open={open} onHide={onClose} variant="large">
      <TitleBar title="Ajouter une image principale">
        ...
        <button tone="critical" onClick={handleCancel}   disabled={!isDirty || isSubmitting}>
          Annuler
        </button>
      </TitleBar>
