...
export function ArticleProvider({ children }) {
  ...
  const [staticData, setStatic] = useState(initalLibs);
  ...
  const prepareFormData = async (fields) => {
    ...
    const processKey = async ([key, value]) => {
      if (value instanceof File) {
        ...
        // Si la valeur est un tableau, traiter chaque élément de manière récursive
        const processedArray = await Promise.all(
          value.map((item) => processKey([key, item])),
        );
  ...
  const submit = async (load, action, body = {}, assign = true) => {
    if (load === "initial") {
      setIsLoading(true);
    }
    ...
    try {
      ...
      if (
        (!fetchedErrors && success) ||
        (Object.keys(fetchedErrors).length === 0 && success)
      ) {
        if (!assign) {
          return response;
        }

       setTheme(response.themes.find((theme) => theme.role === "MAIN"));
  ...
  const loadArticle = async (
    ...
    if (
      ...
      setFields(article);
       ...
       setTheme(response.themes.find((theme) => theme.role === "MAIN"));     setStatic(response);
    } else {
      setErrors(fetchedErrors);
    }
