...
export async function saveFile(file, handle = "tmp", part, name) {
  try {
    ...
    if (!file || !handle || !part || !name) {
      throw new Error(
        "Paramètres manquants : 'file', 'handle', 'part' ou 'name'.",
      );
    }
    ...
    while (locksFile.has(handle)) {
      console.log(
        `En attente : un autre processus traite le handle "${handle}".`,
      );
    ...
    if (typeof file === "string" && file.startsWith("http")) {
      ...
      if (!response.ok) {
        throw new Error(
          `Échec du téléchargement depuis l'URL : ${response.statusText}`,
        );
      }
