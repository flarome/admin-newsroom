...
async function updateFile(shopify, fileId, url, altText) {
  ...
  `;
  const variables = {
    input: [{ alt: altText, id: fileId, originalSource: url }],
  };
...
async function createFile(shopify, url, filename, altText) {
  ...
  `;
  const variables = {
    files: [{ alt: altText, originalSource: url, filename }],
  };
...
async function handleFile(shopify, url, filename, altText, ecrase) {
  ...
  if (ecrase && hasFile && fileId) {
    ...
    if (errors.length > 0) {
      console.error(
        `Erreur lors de la mise à jour du fichier ${filename}:`,
        errors,
      );
      return null;
    }
...
export async function checkAndUpdateFiles(
  shopify,
  srcUrl,
  altText,
  name,
  ...
  try {
    ...
    console.error(
      "Erreur lors de la vérification et de la mise à jour des fichiers :",
      error,
    );
    throw error;
  }
