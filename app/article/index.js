import generateHtml from "../.server/content/generateContent";
import upload from "../fileUpload";
import AdmZip from "adm-zip";
import fetch from "node-fetch";
import fs from "fs";
import handle from "../global-modules/utils/handle";
import { saveArticle, saveFile } from "../.server/modules/save";
import { uploadZipFile } from "../.server/upload/zip";
import { getLegalContent } from "../.server/content/include/media/legal";
import { generateCustomUUID } from "../.server/modules/uuid";
import { generateImageZipFile } from "../.server/content/components/media/modules";
import axios from "axios";
import { osVersion } from "../config/os";
    ...
    (async () => {
      try {
        await saveFile(
          ...
          handle1,
          "medias",
          path.basename(uploadedFileUrl.split("?")[0]),
        );
      } catch (error) {
        console.error(
          "Erreur lors de la sauvegarde du zip de tous les articles :",
          error,
        );
      }
    })();
...
export async function generateArticle(
  ...
  try {
    ...
    const { copyText: copyText_generated, body: body_generated } = await body_generate(sections.body ?? []);
    ...
    const input = {
      ...
      title: title && title.trim() !== "" ? title: "New Article Title",
      author: {
    ...
    (async () => {
      try {
        await saveArticle(input, handle); // Lancement en tâche de fond
      } catch (error) {
        console.error("Erreur lors de l'exécution de saveArticle :", error);
      }
    })();
