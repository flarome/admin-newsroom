  ...
  try { 
    ...
    if (make) {
      ...
      const actionPromises = Object.entries(actionConfig.get || {}).map(async ([key, getAction]) => {
        ...
        const mutation290 = await getAction.mutation();

        const { mutation, variables, mutationName, fetchMode } = mutation290(body, blogId, themeId);
    ...
    if (type === "return") {
      const mutation290 = await build();
      // Construction de la réponse finale via le builder configuré
      return mutation290(response, userErrors, body, errors);
    } else if (type === "rePost") {
